...
public class Contest154B {
  ...
  public int minRemove(int[] measure, int start, int end) {
    ...
    // System.out.println((double)( measure[end] / 2 - average) + " " + (average
    // - (double) measure[start] * 2));
    //
    // if((double) measure[end] / 2 - average > average - (double)
    // measure[start] * 2)
    // return minRemove(measure, start, end - 1, total - measure[end]);
    // else if((double) measure[end] / 2 - average > average - (double)
    // measure[start] * 2)
    // return minRemove(measure, start + 1, end, total - measure[start]);
    // else
    // return Math.min(minRemove(measure, start + 1, end, total -
    // measure[start]), minRemove(measure, start, end - 1, total -
    // measure[end]));
  }
