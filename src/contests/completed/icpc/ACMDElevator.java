...
{	
	...
	public int solve(int start, int goal, int floors, int up, int down)
	{
		...
		if(start + up < floors && start - down >= 0 && num + 1 < moves[start + up] && num + 1 < moves[start - down])
		{
			moves[start + up] = num + 1;
			moves[start - down] = num + 1;
			return Math.min(solve(start + up, goal, floors, up, down), solve(start - down, goal, floors, up, down));
		}
