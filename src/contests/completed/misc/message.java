...
public class message {
  static Map<Integer, List<Long>> buffers = new HashMap<Integer, List<Long>>();
  ...
  public static void PutLL(int target, long value) {
    if (!buffers.containsKey(target)) {
      buffers.put(target, new ArrayList<Long>());
    }
    buffers.get(target).add(value);
  }
  ...
  public static long GetLL(int source) {
    return buffers.get(source).remove(0);
  }
}
